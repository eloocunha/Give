...
router.get('/lista/:categoria', async (req, res) => {
        const empresas = await Microempresa.findAll({attributes: ['id', 'nome_empresa', 'cnpj', 'cep', 'categoria', 'telefone', 'whatsapp', 'email', 'site', 'instagram', 'facebook', 'descricao', 'foto_perfil'], where: {categoria: req.params.categoria}});
...
router.get('/lista', async (req, res) => {
        try {
          const [results, metadata] = await db.sequelize.query("SELECT id, nome_empresa, cep, categoria, telefone, whatsapp, email, site, instagram, facebook, descricao, foto_perfil from microempresas");
          res.json(results);
        } catch (error) {
